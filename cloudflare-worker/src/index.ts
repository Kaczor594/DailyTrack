interface Env {
  DB: D1Database;
  SYNC_TOKEN: string;
}

export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    // CORS headers
    const corsHeaders = {
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, DELETE, OPTIONS",
      "Access-Control-Allow-Headers": "Content-Type, Authorization",
    };

    if (request.method === "OPTIONS") {
      return new Response(null, { headers: corsHeaders });
    }

    // Auth check
    const authHeader = request.headers.get("Authorization");
    const token = authHeader?.replace("Bearer ", "");
    if (token !== env.SYNC_TOKEN) {
      return Response.json({ error: "Unauthorized" }, { status: 401, headers: corsHeaders });
    }

    const url = new URL(request.url);
    const path = url.pathname;

    try {
      // GET /sync?since=<ISO8601> — pull changes since timestamp
      if (request.method === "GET" && path === "/sync") {
        const since = url.searchParams.get("since") || "1970-01-01T00:00:00Z";

        const tasks = await env.DB.prepare(
          "SELECT * FROM tasks WHERE updated_at > ?"
        ).bind(since).all();

        const entries = await env.DB.prepare(
          "SELECT * FROM daily_entries WHERE updated_at > ?"
        ).bind(since).all();

        return Response.json({
          tasks: tasks.results,
          entries: entries.results,
          server_time: new Date().toISOString(),
        }, { headers: corsHeaders });
      }

      // POST /sync — push changes (upsert)
      if (request.method === "POST" && path === "/sync") {
        const body = await request.json() as {
          tasks?: any[];
          entries?: any[];
        };

        const now = new Date().toISOString();
        let tasksUpserted = 0;
        let entriesUpserted = 0;

        // Upsert tasks
        if (body.tasks) {
          for (const task of body.tasks) {
            // Only update if incoming is newer
            const existing = await env.DB.prepare(
              "SELECT updated_at FROM tasks WHERE id = ?"
            ).bind(task.id).first();

            if (!existing || task.updated_at > (existing as any).updated_at) {
              await env.DB.prepare(`
                INSERT INTO tasks (id, name, benchmark, unit, weight, is_cumulative, is_checkbox, sort_order, is_active, created_at, updated_at, deleted)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                  name = excluded.name,
                  benchmark = excluded.benchmark,
                  unit = excluded.unit,
                  weight = excluded.weight,
                  is_cumulative = excluded.is_cumulative,
                  is_checkbox = excluded.is_checkbox,
                  sort_order = excluded.sort_order,
                  is_active = excluded.is_active,
                  updated_at = excluded.updated_at,
                  deleted = excluded.deleted
              `).bind(
                task.id, task.name, task.benchmark, task.unit, task.weight,
                task.is_cumulative ? 1 : 0, task.is_checkbox ? 1 : 0,
                task.sort_order, task.is_active ? 1 : 0,
                task.created_at, task.updated_at, task.deleted ? 1 : 0
              ).run();
              tasksUpserted++;
            }
          }
        }

        // Upsert entries
        if (body.entries) {
          for (const entry of body.entries) {
            const existing = await env.DB.prepare(
              "SELECT updated_at FROM daily_entries WHERE id = ?"
            ).bind(entry.id).first();

            if (!existing || entry.updated_at > (existing as any).updated_at) {
              await env.DB.prepare(`
                INSERT INTO daily_entries (id, task_id, date, value, notes, created_at, updated_at, deleted)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                  task_id = excluded.task_id,
                  date = excluded.date,
                  value = excluded.value,
                  notes = excluded.notes,
                  updated_at = excluded.updated_at,
                  deleted = excluded.deleted
              `).bind(
                entry.id, entry.task_id, entry.date, entry.value,
                entry.notes || null, entry.created_at, entry.updated_at,
                entry.deleted ? 1 : 0
              ).run();
              entriesUpserted++;
            }
          }
        }

        return Response.json({
          tasks_upserted: tasksUpserted,
          entries_upserted: entriesUpserted,
          server_time: now,
        }, { headers: corsHeaders });
      }

      // DELETE /tasks/:id — soft delete
      if (request.method === "DELETE" && path.startsWith("/tasks/")) {
        const id = path.split("/tasks/")[1];
        const now = new Date().toISOString();
        await env.DB.prepare(
          "UPDATE tasks SET deleted = 1, updated_at = ? WHERE id = ?"
        ).bind(now, id).run();
        await env.DB.prepare(
          "UPDATE daily_entries SET deleted = 1, updated_at = ? WHERE task_id = ?"
        ).bind(now, id).run();
        return Response.json({ ok: true, server_time: now }, { headers: corsHeaders });
      }

      // DELETE /entries/:id — soft delete
      if (request.method === "DELETE" && path.startsWith("/entries/")) {
        const id = path.split("/entries/")[1];
        const now = new Date().toISOString();
        await env.DB.prepare(
          "UPDATE daily_entries SET deleted = 1, updated_at = ? WHERE id = ?"
        ).bind(now, id).run();
        return Response.json({ ok: true, server_time: now }, { headers: corsHeaders });
      }

      return Response.json({ error: "Not found" }, { status: 404, headers: corsHeaders });

    } catch (err: any) {
      return Response.json({ error: err.message }, { status: 500, headers: corsHeaders });
    }
  },
};
