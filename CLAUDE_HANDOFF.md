...
- **Per-task week-start anchor** — weekly cumulative tasks can override the locale-default week boundary. E.g., a task anchored to Friday rolls over Thu→Fri regardless of device locale. nil anchor preserves locale-default behavior. Monthly/yearly anchors not yet exposed (data model supports it; UI is weekly-only for now)
...
- **Period window logic**: `periodWindow(for:on:)` is a single free function in `Shared/PeriodWindow.swift` (compiled into both the app and widget targets via the `PBXFileSystemSynchronizedRootGroup` exception set). It uses `Calendar.dateInterval(of:for:)` for locale-aware boundaries, except when `task.cumulativePeriod == "week"` and `task.periodAnchor` is set (1=Sun..7=Sat per `Calendar.weekday`), in which case it returns a 7-day window starting on the anchor weekday.
...
**Session 2026-04-21 (this session):**
- **Per-task week-start day feature.** Added `periodAnchor: Int?` field to `TaskDefinition` and `CodableTaskDefinition` (nil = locale default, 1=Sun..7=Sat per `Calendar.weekday`). SwiftData lightweight migration works because the field is optional.
- **Extracted `periodWindow(for:on:)`** into `DailyTrack/Shared/PeriodWindow.swift` as a single free function. Deleted the three duplicates in `DailyViewModel`, `HistoryViewModel`, and `DailyTrackWidget`. Added file to widget target via `project.pbxproj` exception set.
- **Anchor-aware weekly window logic.** When `cumulativePeriod == "week"` and `periodAnchor` is set, `periodWindow` computes the 7-day window manually (most recent anchor weekday ≤ date + 6 days).
- **Settings UI.** New "Week Starts On" picker in `SettingsView` task editor (shown only for weekly tasks). Options: Locale Default + Sun..Sat. Anchor is cleared when period changes off weekly.
- **Cloudflare Worker.** Added `ensurePeriodAnchorColumn` auto-migration (nullable INTEGER), updated `INSERT ... ON CONFLICT` to write `period_anchor`, added column to `schema.sql`. Client omits `period_anchor` from push payload when nil (preserves null on server).
- **Sync merge.** Same defensive pattern as `cumulativePeriod` — pull only overwrites local `periodAnchor` when server sends a value, so old Workers don't reset the field.
- Tested end-to-end on Mac; iOS/iPad testing deferred to a future session.
- Localizable.xcstrings auto-regenerated by Xcode to include the new "Week Starts On" / weekday strings.
...
- **Worker not yet deployed with `period_anchor`** as of end of this session. `npm run deploy` is required before cross-device testing of anchored weekly windows will work. Auto-migration runs on first `/sync` hit post-deploy — no manual `ALTER TABLE`.
- **Monthly/yearly anchors not yet exposed in UI.** Data model stores `periodAnchor` generically but only the weekly path reads it. Future pass can add day-of-month and month+day pickers using the same field.
- **SourceKit LSP noise in this repo.** Claude Code's SourceKit integration misreports "Cannot find type X" on valid pre-existing code (e.g., `TaskDefinition`, `DailyEntry`, `AppGroupContainer`) because `PBXFileSystemSynchronizedRootGroup` isn't reconciled the way Xcode's build does. Rely on Xcode builds to verify, not LSP diagnostics.
...
- [ ] Deploy Worker (`cd cloudflare-worker && npm run deploy`) and verify anchored weekly window round-trips iPhone ↔ Mac (set Claude usage task to Friday start, confirm window stays Fri–Thu across devices)
...
- [ ] Expose monthly day-of-month anchor and yearly month+day anchor in the task editor (data model already supports it via `periodAnchor`)
- [ ] Update `SeedData.swift` to match current task definitions (low priority — first-sync server-wins masks the staleness)
